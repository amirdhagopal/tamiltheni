...
async function generateSentence() {
    ...
    if (aiBtn) {
        aiBtn.disabled = true;
        aiBtn.innerHTML = `<span>⏳</span> Generating...`;
    }
    ...
    try {
        ...
        if (aiBtn) {
            ...
            aiBtn.innerHTML = `<span>✨</span> Generate Sentence`;
        }
    }
...
function updateUI(shouldSpeak = true) {
    ...
    if (aiBtn) {
        aiBtn.disabled = false;
        aiBtn.innerHTML = '<span>✨</span> Generate Sentence with AI';
    }
...
export function init() {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'ArrowRight' || ((e.key === ' ' || e.key === 'Enter') && !isInteractive)) {
            // Prevent default scrolling for Space if we are handling it
